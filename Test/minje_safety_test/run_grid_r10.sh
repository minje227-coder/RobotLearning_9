#!/usr/bin/env bash
# VLSA {0,1,2} x delay x seed grid, but with policy-replan-horizon=10 (re-plan every 10 steps).
...
runjob() {
  ...
  CUDA_VISIBLE_DEVICES=$gpu MUJOCO_GL=egl python minje_run_safety_eval.py \
    ...
    --policy-replan-horizon 10 --trajectory-horizon 0 \
    --out-csv "results/grid_replan10/${lab}.csv" --out-json "results/grid_replan10/${lab}.json" \
    --label "$lab" > "results/grid_replan10/${lab}.log" 2>&1
}
jobs=()
for vl in 0 1 2; do for d in $DELAYS; do jobs+=("$vl:$d"); done; done
for g in 0 1 2 3 4 5; do
  ( idx=$g; while [ $idx -lt ${#jobs[@]} ]; do job=${jobs[$idx]}; runjob $g "${job%%:*}" "${job##*:}"; idx=$((idx+6)); done ) &
done
wait
echo GRID_R10_DONE
