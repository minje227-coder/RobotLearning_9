#!/usr/bin/env bash
# VLSA {0,1,2} x delay {7.5..12} x seed {0..49} grid. replan-horizon=50. metrics only.
...
runjob() {
  ...
  CUDA_VISIBLE_DEVICES=$gpu MUJOCO_GL=egl python minje_run_safety_eval.py \
    ...
    --out-csv "results/grid_s50/${lab}.csv" --out-json "results/grid_s50/${lab}.json" \
    --label "$lab" > "results/grid_s50/${lab}.log" 2>&1
}
jobs=()
