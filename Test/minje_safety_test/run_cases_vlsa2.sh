#!/usr/bin/env bash
# Render one VLSA-2 video per (delay, case-category). Filled+intervene overlay.
...
OUT=$HOME/workspace/RobotLearning_9/Test/minje_safety_test/cases_vlsa2
mkdir -p "$OUT"
...
render() {  # $1=gpu  $2="dlabel dval s..."
  ...
  CUDA_VISIBLE_DEVICES=$gpu MUJOCO_GL=egl python minje_run_safety_eval.py \
    ...
    --max-steps 500 --robot1-start-delay-sec "$dv" --safety-arms 0 1 \
    --policy-replan-horizon 50 --trajectory-horizon 0 \
    --video-camera all --video-seeds $seeds --video-dir "$OUT" \
    --out-csv "/tmp/case_v2_d${dl}.csv" --out-json "/tmp/case_v2_d${dl}.json" \
    --label "CASE_V2_d${dl}" > "$OUT/render_d${dl}.log" 2>&1
}
