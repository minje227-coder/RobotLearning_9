...
ROBOT0_PASSIVE_GRIPPER_ACTION = -1.0   # robot0 gripper 열어둠 (반전 적용)
...
JOINT_TOL_MID = 0.2             # 중간 보간점(_mid)은 빡세게 안 가도 되게 — 큰 값일수록 빨리 통과
...
parser.add_argument(
    "--video-out",
    type=str,
    default=str(pathlib.Path(__file__).with_name("AUTO_SCENE2.mp4")),
)
...
pregrasp_qpos = solve_ik_for_pose(env.env.sim, robot1, milk_above_xyz, target_ori_mat, home_qpos)
grasp_qpos = solve_ik_for_pose(env.env.sim, robot1, milk_pick_xyz, target_ori_mat, pregrasp_qpos)
postgrasp_lift_qpos = solve_ik_for_pose(env.env.sim, robot1, postgrasp_lift_xyz, target_ori_mat, grasp_qpos)
lift_qpos = solve_ik_for_pose(env.env.sim, robot1, milk_above_xyz, target_ori_mat, postgrasp_lift_qpos)
preplace_qpos = solve_ik_for_pose(env.env.sim, robot1, trash_above_xyz, target_ori_mat, lift_qpos)
preplace_mid_qpos = interpolate_joint_waypoint(lift_qpos, preplace_qpos)
place_qpos = solve_ik_for_pose(env.env.sim, robot1, trash_target_xyz, target_ori_mat, preplace_qpos)
retreat_qpos = solve_ik_for_pose(env.env.sim, robot1, retreat_xyz, target_ori_mat, place_qpos)
...
for _name, _xyz, _q, _ori_target in [
    ...
    print_waypoint_ik_debug(env.env.sim, robot1, _name, _xyz, _ori_target, _q)
    _ori = get_grip_site_orientation_for_qpos(env.env.sim, robot1, _q)
    print(f"{_name}_eef_z_axis_world={_ori[:, 2].tolist()}  eef_x={_ori[:, 0].round(2).tolist()}")

IK_REFERENCE_NAME = robot1.gripper.important_sites["grip_site"]
...
post_grasp_sequence = [
    ...
    ("move_preplace_mid", preplace_mid_qpos,  GRIPPER_CLOSE),
...
for _ in range(MAX_STEPS):
    ...
    if phase == "open_then_approach":
        ...
        if joint_distance_to(env, target_qpos) < _tol or _timed_out:
            if _timed_out:
                print(f"[timeout] {phase}: jd={joint_distance_to(env, target_qpos):.3f}, advancing")
            move_index += 1
        ...
        robot1_action = make_joint_position_action(_home_target, cur, gripper_cmd=GRIPPER_OPEN)
    ...
    action = make_dual_action(robot1_action)
