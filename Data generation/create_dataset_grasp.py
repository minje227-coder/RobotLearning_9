...
TASK_DESCRIPTION = "put the milk into the target area inside the trash can"
...
ROBOT1_PASSIVE_GRIPPER_ACTION = -1.0
...
DATASET_CAMERAS = ["sideview_robot0_left", "robot0_eye_in_hand", "sideview_robot0_right"]
DEBUG_CAMERAS = []
...
def extract_state(obs):
    return np.concatenate(
        [
            ...
            [float(np.mean(obs["robot0_gripper_qpos"]))],
        ]
    ).astype(np.float32)
...
def make_frame(obs, robot0_action, image_rotation):
    side_left_img = preprocess_image(obs["sideview_robot0_left_image"], image_rotation)
    wrist_img = preprocess_image(obs["robot0_eye_in_hand_image"], image_rotation)
    side_right_img = preprocess_image(obs["sideview_robot0_right_image"], image_rotation)
    state = extract_state(obs)
    action = np.asarray(robot0_action, dtype=np.float32)
    if state.shape != (8,):
...
def make_debug_frame(obs, image_rotation):
    images = [
        preprocess_image(obs["sideview_robot0_left_image"], image_rotation),
        preprocess_image(obs["robot0_eye_in_hand_image"], image_rotation),
        preprocess_image(obs["sideview_robot0_right_image"], image_rotation),
    ]
...
def solve_waypoints(env, obs):
    ...
    pregrasp_qpos = solve_ik_for_pose(sim, robot0, milk_above_xyz, target_ori_mat, home_qpos)
    grasp_qpos = solve_ik_for_pose(sim, robot0, milk_pick_xyz, target_ori_mat, pregrasp_qpos)
    postgrasp_lift_qpos = solve_ik_for_pose(sim, robot0, postgrasp_lift_xyz, target_ori_mat, grasp_qpos)
    lift_qpos = solve_ik_for_pose(sim, robot0, milk_above_xyz, target_ori_mat, postgrasp_lift_qpos)
    preplace_qpos = solve_ik_for_pose(sim, robot0, trash_above_xyz, target_ori_mat, lift_qpos)
    place_qpos = solve_ik_for_pose(sim, robot0, trash_target_xyz, target_ori_mat, preplace_qpos)
    retreat_qpos = solve_ik_for_pose(sim, robot0, retreat_xyz, target_ori_mat, place_qpos)
    return {
...
def run_scripted_episode(env, obs, max_steps, image_rotation, save_debug_video, fps):
    ...
    for key in ["sideview_robot0_left_image", "robot0_eye_in_hand_image", "sideview_robot0_right_image"]:
        if key not in obs:
    ...
    post_grasp_index = 0
    retreat_index = 0
    grasp_hold_steps = max(1, int(round(fps * GRASP_HOLD_SECONDS)))
    record_frames = False

    for _ in range(max_steps):
        phase_at_step_start = phase
        ...
        if phase == "open_then_approach":
            ...
            if close_hold_steps >= GRIP_CLOSE_HOLD_STEPS:
                phase = "grasp_hold"
                phase_steps = 0
        elif phase == "grasp_hold":
            robot0_action = make_joint_position_action(cur, cur, gripper_cmd=GRIPPER_CLOSE)
            if phase_steps >= grasp_hold_steps:
                post_grasp_index = 0
            ...
            robot0_action = make_joint_position_action(waypoints["home"], cur, gripper_cmd=GRIPPER_OPEN)
        ...
        if phase_at_step_start == "grasp_hold":
            record_frames = True

        if record_frames:
            frames.append(make_frame(obs, robot0_action, image_rotation))
            if save_debug_video:
                debug_frames.append(make_debug_frame(obs, image_rotation))

        obs, _, _, _ = env.step(make_dual_action(robot0_action))
        lock_robot1_pose(env.env.sim, env.env.robots[1])
        obs = env.env._get_observations()
        phase_steps += 1
        if done:
